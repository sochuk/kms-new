...
{
    [NeedAccessRight]
    public partial class CompareUID : CPanel
    {
        ...
        public void RebuildCARDUID(IHubContext hub, string username)
        {
            using (OracleConnection connection = new OracleConnection(Database.getConnectionString("Default")))
            {
                ...
                using (OracleCommand command = new OracleCommand())
                {
                    ...
                    command.CommandText = @"DROP TABLE CARD_CARDUID CASCADE PURGE";
                    ...
                    command.Parameters.Clear();
                    command.CommandText = @"
                        CREATE TABLE CARD_CARDUID(CARDUID PRIMARY KEY, TOTAL) 
                        TABLESPACE CMS_CARDUID AS 
                        SELECT CARDUID, COUNT(*) TOTAL FROM CARD
                        WHERE CARDUID IS NOT NULL
                        GROUP BY CARDUID";
                    hub.Clients.User(username).started();
                    command.ExecuteNonQuery();
        ...
        protected void CSVCARDUIDExport_Click(object sender, EventArgs e)
        {
            var hub = GlobalHost.ConnectionManager.GetHubContext<IISLogHub>();
            string username = M_User.getUsername();
        ...
        protected void ExcelCARDUIDExport_Click(object sender, EventArgs e)
        {
            ExportXLSX3(grid);
        }
        ...
        protected void CARDUID_Selecting(object sender, LinqServerModeDataSourceSelectEventArgs e)
        {
            var db = new KMSContext();
            e.KeyExpression = "ID";
            e.QueryableSource = db.CARD_CARDUID;
        }

        protected void grid_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
        {
            var dxGrid = (ASPxGridView)sender;
            var item = dxGrid.GetRowValues(e.VisibleIndex, "CARDUID");
            dt_temp = Database.getDataTable($"SELECT * FROM CARD WHERE CARDUID='{item}'");
